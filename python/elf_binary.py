...
class BasicBlock(BinaryObject):
    ...
    def __init__(self, angr_bb):
        print ("type angr_bb = " + str(type(angr_bb)))
...
class ElfBinary:
    ...
    def get_section(self, section_name):
        ...
        content = self.angr_proj.loader.memory.load(sec.vaddr, sec.memsize)
    ...
    def get_bb(self, addr):
        return BasicBlock(self.angr_proj.factory.block(int(addr)))
    ...
    def get_accurate_bb(self, addr, thresh=10, num_tries=100):
        ...
        out_block = block
        for cur_addr in range(int(addr) - 1, max(int(addr) - num_tries, self.min_addr), -1):
...
if __name__ == "__main__":
    bin = ElfBinary('../testbench/bin/hash/sha1.o')
